...
export default class Carousel extends Component {
  ...
  initForDrag = () => {
    if (this.props.draggable) {
      ...
      item.addEventListener('mousedown', (e) => {
        this.handleClearIntervalAutoPlay();
        startetDragPos = e.clientX;
        return window.addEventListener('mousemove', this.mouseMove);
      }));
      window.addEventListener('mouseup', (e) => {
        endedeDragPos = e.clientX;
        window.removeEventListener('mousemove', this.mouseMove);
        if ((startetDragPos < endedeDragPos) && (startetDragPos > (this.state.wrapperWidth / 2))) {
          this.handleChangeSlide(this.state.selectedItem - 1);
        } else {
          this.handleChangeSlide(this.state.selectedItem + 1);
        }
      });
  ...
  render() {
    ...
    if (this.state.clientX) {
      const vv = `${((position - this.state.clientX))}px`;
